...
export default function ShedSignup({ navigation }) {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.passwordWrap}>
        <TextInput style={[styles.input, styles.passwordInput]} secureTextEntry={!showPassword} onChangeText={(t) => setForm({...form, pass: t})} />
      ...
      <View style={styles.passwordWrap}>
        <TextInput style={[styles.input, styles.passwordInput]} secureTextEntry={!showConfPassword} onChangeText={(t) => setForm({...form, conf: t})} />
        <TouchableOpacity style={styles.passwordIconBtn} onPress={() => setShowConfPassword((prev) => !prev)}>
          <Image source={require('../../assets/viewpassword.png')} style={styles.passwordIcon} />
        </TouchableOpacity>
      </View>
  ...
  <TouchableOpacity 
    style={[styles.btnSm, doc && styles.btnSmSelected]} 
    onPress={async () => {
      let r = await DocumentPicker.getDocumentAsync({type: 'application/pdf'});
      if(!r.canceled) setDoc(r.assets[0]);
    }}
  >
    <Text style={[styles.bold, doc && styles.boldSelected]}>{doc ? "Selected" : "Upload"}</Text>
  </TouchableOpacity>
</View>
